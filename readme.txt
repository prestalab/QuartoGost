...
- вместе с презентацией автоматически формируется Word-документ раздаточных
  материалов `*-handout.docx` по логике `presentation_handout.tex`.
...
- после сборки автоматически создаётся `*-handout.docx` с двумя слайдами на
  странице A4 и комментариями;
...
Комментарии для раздаточных материалов:

- комментарий к титульному слайду задаётся в YAML front matter:
  `notes: | ...`;
- комментарий к обычному слайду задаётся внутри слайда блоком:
  `::: {.notes} ... :::`;
- `pptx` и `handout.docx` собираются из одного и того же `qmd`.
